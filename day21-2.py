...
def un_rotateLR(instruction, password):
    # rotate left/right X steps : the whole string should be rotated; for example, e.g right 1, abcd -> dabc.
    # rotate left 1
    tokens = instruction.split(' ')
    direction, y = tokens[1], int(tokens[2])
...
def un_rotatePosition(instruction, password):
    # to unrotate the string, left rotate it until you get a string that could be produced from
    # left rotated string
    original = ''.join(password)
    tokens = instruction.split(' ')
    x = tokens[6]
    for i in range(len(password)):
        front = list(password.pop(0))
        password = password + front
        if ''.join(rotatePosition(instruction, list(password))) == original:
            return password
    return password
...
def un_moveXY(instruction, password):
    # move position X to position Y : the letter which is at index X should be removed from the string,
    # then inserted such that it ends up at index Y.
    #move position 1 to position 4
    #move position 3 to position 0
...
functionMap = {
    ...
    'rotate left' : un_rotateLR,
    'rotate right' : un_rotateLR,
    'move position' : un_moveXY,
    'rotate based' : un_rotatePosition
}
